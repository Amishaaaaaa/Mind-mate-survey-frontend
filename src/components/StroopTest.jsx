...
//       const congruentAvgRT = congruentRTs.length > 0 ? (congruentRTs.reduce((a, b) => a + b, 0) / congruentRTs.length) : 0;
//       const incongruentAvgRT = incongruentRTs.length > 0 ? (incongruentRTs.reduce((a, b) => a + b, 0) / incongruentRTs.length) : 0;
...
//                           : '0.0'}%</p>
//                         <p>Avg. Reaction Time: {(intermediateScore.congruent.avgRT.toFixed(2))}ms</p>
//                       </div>
...
//                           : '0.0'}%</p>
//                         <p>Avg. Reaction Time: {(intermediateScore.incongruent.avgRT.toFixed(2))}ms</p>
//                       </div>
//                     </div>
...
//                   <div className="result-stat"><b>Word Reading: </b> <b>Correct: </b><span className="result-sub">{finalResults.word.score}/{testResults.word.total} items, <b>Avg RT:</b> {finalResults.word.rt.toFixed(2)}ms, <b>Accuracy:</b> {finalResults.word.acc.toFixed(1)}%</span></div>
//                   <div className="result-stat"><b>Color Naming: </b> <b>Correct: </b><span className="result-sub">{finalResults.color.score}/{testResults.color.total} items, <b>Avg RT:</b> {finalResults.color.rt.toFixed(2)}ms, <b>Accuracy:</b> {finalResults.color.acc.toFixed(1)}%</span></div>
                  
//                   <div className="result-stat"><b>Color-Word (Congruent): </b> <b>Correct: </b><span className="result-sub">{finalResults.colorWord.congruent.score}/{testResults.colorWord.congruent.total} items, <b>Avg RT:</b> {finalResults.colorWord.congruent.rt.toFixed(2)}ms, <b>Accuracy:</b> {finalResults.colorWord.congruent.acc.toFixed(1)}%</span></div>
//                   <div className="result-stat"><b>Color-Word (Incongruent): </b> <b>Correct: </b><span className="result-sub">{finalResults.colorWord.incongruent.score}/{testResults.colorWord.incongruent.total} items, <b>Avg RT:</b> {finalResults.colorWord.incongruent.rt.toFixed(2)}ms, <b>Accuracy:</b> {finalResults.colorWord.incongruent.acc.toFixed(1)}%</span></div>
//                 </div>
...
//   // Add trial sequence for exact control
...
//       newColor = getRandomItem(colors);
//     } else if (currentTest === 3) {
//       // Use pre-determined sequence for exact control
//       congruent = trialSequence[trialIndex];
...
//       setCongruentTrialsLeft(30);
//       setIncongruentTrialsLeft(30);
//       // Create and set the trial sequence
//       const sequence = createTrialSequence();
...
//   }, [currentTest, currentTrial, totalItems, maxTrials, currentWord, currentColor, isCongruent, trialIndex]);
...
function StroopTest({ isLoggedIn, onLogout }) {
  ...
  const handleItemTimeout = () => {
    ...
    // Don't include timeout RT in calculations - just increment totals
    let correctAnswer = '';
    ...
    if (currentTest === 3) {
      if (isCongruent) {
        congruentStatsRef.current.total += 1;
        setCongruentTrialsLeft(prev => prev - 1);
        // Don't add timeout RT to response times array
      } else {
        incongruentStatsRef.current.total += 1;
        setIncongruentTrialsLeft(prev => prev - 1);
        // Don't add timeout RT to response times array
    ...
    // Don't add timeout RT to response times for tests 1 and 2 either
  ...
  const startTest = () => {
    ...
    if (currentTest === 3) {
      setCongruentTrialsLeft(15);
      setIncongruentTrialsLeft(15);
      // Create and set the trial sequence
      const sequence = createTrialSequence();
      setTrialSequence(sequence);
      setTrialIndex(0);
  ...
  useEffect(() => {
    const handleKeyPress = (event) => {
      if (["KeyR", "KeyG", "KeyB", "KeyY"].includes(event.code) && 
        ...
        // Only record response time for CORRECT answers
        if (currentTest === 3) {
          if (isCongruent) {
            congruentStatsRef.current.total += 1;
            if (isCorrect) {
              congruentStatsRef.current.correct += 1;
              // Only add RT for correct responses
              setResponseTimes((prev) => {
                const newArr = [...prev];
                newArr[currentTest - 1].congruent = [...newArr[currentTest - 1].congruent, rt];
                responseTimesRef.current = newArr;
            ...
            incongruentStatsRef.current.total += 1;
            if (isCorrect) {
              incongruentStatsRef.current.correct += 1;
              // Only add RT for correct responses
              setResponseTimes((prev) => {
                const newArr = [...prev];
                newArr[currentTest - 1].incongruent = [...newArr[currentTest - 1].incongruent, rt];
          ...
          // Only add RT for correct responses in tests 1 and 2
          if (isCorrect) {
            setResponseTimes((prev) => {
              const newArr = [...prev];
              newArr[currentTest - 1] = [...newArr[currentTest - 1], rt];
          ...
          <div id="final-results" className={`final-results${showFinalResults ? '' : ' hidden'}`} style={{ display: showFinalResults ? 'block' : 'none', textAlign: 'left' }}>
            {showFinalResults && finalResults && (
              <>
                ...
                <div style={{ marginLeft: '50px'}}>
                  <div className="result-stat"><b>Word Reading: </b> <span className="result-sub"><b>Avg RT:</b> {finalResults.word.rt.toFixed(2)}ms</span></div>
                  <div className="result-stat"><b>Color Naming: </b> <span className="result-sub"><b>Avg RT:</b> {finalResults.color.rt.toFixed(2)}ms</span></div>
                  
                  <div className="result-stat"><b>Color-Word (Congruent): </b> <span className="result-sub"><b>Avg RT:</b> {finalResults.colorWord.congruent.rt.toFixed(2)}ms</span></div>
